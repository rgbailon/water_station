...
export function exportOrdersExcel(orders) {
  ...
  const summary = [
    ...
    { label: 'Unpaid Orders', value: String(unpaidOrders) },
    { label: 'Unpaid Revenue', value: peso(unpaidRevenue) },
  ...
  const html = buildWorkbookSheet({
    ...
    subtitle: `${orders.length} orders • ${peso(totalRevenue)} total sales • ${totalBorrowed} gals borrowed • ${paidOrders} paid / ${unpaidOrders} unpaid • All orders from this device`,
    headers, rows, summary,
    footer: 'Order status is database-driven. Borrowed audited in orders.borrowed_count • Payment audited in orders.payment_status.',
  })
